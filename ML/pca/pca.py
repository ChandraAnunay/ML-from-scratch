...
class PCA:
    ...
    def transform(self,X):
        # centralizing the data
        X = X - self.mean
        self.principal_components = np.matmul(X,self.eigenvec)
        self.projection = np.matmul(self.principal_components,self.eigenvec.T)
        return self.principal_components, self.projection + self.mean
...
#         self.eigenvec = eigenvec[:, idx[:self.n_components]]
    
#     def transform(self, X):
#         X_centered = X - self.mean
#         self.principal_components = np.dot(X_centered, self.eigenvec)
#         return self.principal_components
    
#     def inverse_transform(self, components):
#         return np.dot(components, self.eigenvec.T) + self.mean
